...
function getGrouped(workSheet, colsInfo, rowCount) {
    return new Array(rowCount).fill(undefined).map(function (_, index) {
        ...
        colsInfo.forEach(function (k) {
            ...
            // 日期时间处理
            if (dValue && k.dataType === model_1.Type.Date) {
                var date = util_1.getLocalDate(dValue);
                var formats = dValue < 1
                    ? "HH:mm" // < 1 处理为时间
                    : Number.isInteger(dValue)
                        ? "YYYY-MM-DD" // 整数处理为日期
                        : "YYYY-MM-DD HH:mm"; // 有小数处理为日期 + 时间
                dValue = date_fns_1.format(date, formats);
                // log("dValue format: ", dValue);
            }
            //数值处理为字符串 ungrouped
            row[dKey] =
                k.dataType === model_1.Type.Number
                    ? (k.dataType = model_1.Type.String) && String(dValue)
                    : dValue;
        });
        return row;
    });
